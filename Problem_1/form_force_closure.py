...
def wrench(f, p):
    ...
    w = np.hstack((f, np.matmul(cross_matrix(p), f)))
...
def cone_edges(f, mu):
    ...
    if D == 2:
        ...
        fn = f[idx]
        if idx == 0:
            edges[0] = np.array([fn, mu * fn])
            edges[1] = np.array([fn, -mu * fn])
        else:
            edges[0] = np.array([mu * fn, fn])
            edges[1] = np.array([-mu * fn, fn])
    ...
    # Spatial wrenches
        ...
        fx, fy, fz = f
        fn = np.linalg.norm(f)
        # fp1 and fp2 are perpendicular to f
        fp1 = np.array([fz, fz, -fx - fy])
        fp2 = np.matmul(cross_matrix(fp1), f)
        n1 = fp1 / np.linalg.norm(fp1)
        n2 = fp2 / np.linalg.norm(fp2)
        edges[0] = f + mu * fn * n1
        edges[1] = f - mu * fn * n1
        edges[2] = f + mu * fn * n2
        edges[3] = f - mu * fn * n2
...
def form_closure_program(F):
    ...
    # F is full rank
    k = cp.Variable(j)
    constraints = [cp.matmul(F, k) == 0, k >= 1]
...
def is_in_force_closure(forces, points, friction_coeffs):
    ...
    j = len(points)
    l = friction_coeffs.count(0.)  # number of frictionless points
    if forces[0].size == 2:
        n = 3
        F = np.zeros((n, 2 * j - l))
    elif forces[0].size == 3:
        n = 6
        F = np.zeros((n, 4 * j - 3 * l))
    else:
        print("invalid points")
    ...
    for i in range(j):
        ...
        for e in range(len(edges)):
            F[:, count] = wrench(edges[e], points[i])
            count = count + 1
