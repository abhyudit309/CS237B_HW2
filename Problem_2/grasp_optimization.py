...
def grasp_optimization(grasp_normals, points, friction_coeffs, wrench_ext):
    ...
    for m in range(M):
        A = np.zeros((D, D*M+1))
        A[0:D-1, D*m:D*m+D-1] = np.eye(D - 1)
        b = np.zeros(D)
        c = np.zeros(D*M+1)
        c[D*m+D-1] = friction_coeffs[m]
        d = 0
        As.append(A)
        bs.append(b)
        cs.append(c)
        ...
        F[0:D, D*m:D*m+D] = transformations[m]
        F[D:, D*m:D*m+D] = np.matmul(cross_matrix(points[m]), transformations[m])

    # ||f_i|| <= s constraints
    for m in range(M):
        A = np.zeros((D, D*M+1))
        A[:, D*m:D*m+D] = np.eye(D)
        b = np.zeros(D)
        c = np.zeros(D*M+1)
        c[-1] = 1
        d = 0
...
def precompute_force_closure(grasp_normals, points, friction_coeffs):
    ...
    F = np.zeros((2*N, M*D))
    unit_wrenches = np.zeros((2*N, N))
    for i in range(N):
        unit_wrenches[2 * i, i] = 1
        unit_wrenches[2 * i + 1, i] = -1
    for j in range(2*N):
        F[j] = np.array(grasp_optimization(grasp_normals, points, friction_coeffs, unit_wrenches[j])).ravel()
    ...
    def force_closure(wrench_ext):
        ...
        f = np.zeros(M*D)
        for k in range(N):
            f = f + max(0, wrench_ext[k]) * F[2*k] + max(0, -wrench_ext[k]) * F[2*k+1]
